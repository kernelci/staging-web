...
class DiscordWebhook:
    ...
    async def send_staging_start(self, user: str, staging_run_id: int):
        ...
        embed = {
            ...
            "fields": [
                {"name": "Initiated by", "value": user, "inline": True},
                {"name": "Run ID", "value": f"#{staging_run_id}", "inline": True},
            ],
        }

        payload = {"embeds": [embed], "username": "KernelCI Staging Bot"}

        await self._send_webhook(payload)

    async def send_staging_complete(
        self,
        user: str,
        staging_run_id: int,
        status: str,
