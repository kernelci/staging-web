...
class DeploymentManager:
    ...
    async def update_api_pipeline(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "error": None,
            "docker_images": {},
            "details": {"steps": [], "start_time": datetime.utcnow().isoformat()},
        }
        ...
        try:
            ...
            if not start_pipeline_result["success"]:
                result["error"] = (
                    f"Pipeline start failed: {start_pipeline_result['error']}"
                )
                return result
    ...
    async def update_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Pull docker images
            pull_result = await self.docker_compose_pull(
                self.compose_files, cwd=self.pipeline_path
            )
            ...
            # Stop containers
            stop_result = await self.docker_compose_down(
                self.compose_files, with_orphans=True, cwd=self.pipeline_path
            )
            ...
            if not stop_result["success"]:
                ...
                # Try stopping again
                stop_retry_result = await self.docker_compose_down(
                    self.compose_files, with_orphans=True, cwd=self.pipeline_path
                )
                ...
                if not stop_retry_result["success"]:
                    return {
                        "success": False,
                        "error": "Failed to stop pipeline containers",
                        "steps": steps,
                    }
    ...
    async def update_api(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pull_result = await self.docker_compose_pull(
                services=self.api_services, cwd=self.api_path
            )
            steps.append({"name": "docker_pull", "result": pull_result})
            ...
            if not stop_result["success"]:
                ...
                if not stop_retry_result["success"]:
                    return {
                        "success": False,
                        "error": "Failed to stop API containers",
                        "steps": steps,
                    }
    ...
    async def start_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            start_result = await self.docker_compose_up(
                compose_files=self.compose_files, env=env, cwd=self.pipeline_path
            )
