...
class KernelTreeManager:
    ...
    async def update_kernel_tree(self, tree: str) -> Dict[str, Any]:
        ...
        if tree not in self.tree_configs:
            return {"success": False, "error": f"Unknown tree: {tree}", "details": {}}
        ...
        try:
            ...
            for line in stdout_str.split("\n"):
                if "commit" in line.lower() and len(line.split()) > 1:
                    ...
                    if len(potential_sha) >= 7 and all(
                        c in "0123456789abcdef" for c in potential_sha[:7]
                    ):
            ...
            return {
                ...
                "tree": tree,
                "details": {"exception": str(e), "config": config},
            }
    ...
    async def get_kernel_tree_status(self, tree: str) -> Dict[str, Any]:
        ...
        try:
            if not os.path.exists(checkout_path):
                return {"exists": False, "tree": tree, "config": config}
