...
class StagingOrchestrator:
    ...
    async def process_staging_runs(self):
        ...
        try:
            ...
            active_runs = (
                db.query(StagingRun)
                .filter(StagingRun.status == StagingRunStatus.RUNNING)
                .all()
            )
            ...
            if len(active_runs) > 1:
                print(
                    f"WARNING: Found {len(active_runs)} concurrent running staging runs!"
                )
                print("This should not happen. Active runs:")
                for run in active_runs:
                    print(
                        f"  - Run #{run.id} by {run.user.username}, started {run.start_time}"
                    )
                ...
                for run in active_runs:
                    if run.id != oldest_run.id:
                        print(
                            f"  - Marking run #{run.id} as failed due to concurrency conflict"
                        )
                        run.status = StagingRunStatus.FAILED
                        run.end_time = datetime.utcnow()
                        run.error_message = (
                            "Cancelled due to concurrent staging run conflict"
                        )
    ...
    async def initialize_staging_steps(self, staging_run: StagingRun, db: Session):
        ...
        steps = [{"type": StagingStepType.GITHUB_WORKFLOW, "order": 1}]
        ...
        if not skip_self_update:
            steps.append({"type": StagingStepType.SELF_UPDATE, "order": order_counter})
            order_counter += 1
    ...
    def get_next_step_to_process(
        self, staging_run: StagingRun
    ) -> Optional[StagingRunStep]:
        """Get the next step that needs processing"""
        # First check if any step is currently running and needs status update
        running_step = next(
            (s for s in staging_run.steps if s.status == StagingStepStatus.RUNNING),
            None,
        )
        if running_step:
            return running_step
        ...
        failed_step = next(
            (s for s in staging_run.steps if s.status == StagingStepStatus.FAILED), None
        )
        if failed_step:
            # Mark all remaining pending steps as skipped
            pending_steps = [
                s for s in staging_run.steps if s.status == StagingStepStatus.PENDING
            ]
    ...
    def all_steps_completed_successfully(self, staging_run: StagingRun) -> bool:
        ...
        return all(
            step.status
            in [
                StagingStepStatus.COMPLETED,
                StagingStepStatus.PARTIAL_SUCCESS,
                StagingStepStatus.SKIPPED,
            ]
    ...
    async def process_github_workflow_step(
        ...
        try:
            ...
            if github_token and github_token.strip():
                ...
                print(
                    f"Initializing/updating GitHub manager with token during step processing"
                )
                self.github_manager = GitHubWorkflowManager(github_token)
            else:
        ...
        if step.status == StagingStepStatus.PENDING:
            ...
            if workflow_run_id:
                ...
                step.details = json.dumps(
                    {"workflow_run_id": workflow_run_id, "status": "triggered"}
                )
                db.commit()
            else:
                step.status = StagingStepStatus.FAILED
            ...
            if step.github_actions_id:
                status = await self.github_manager.get_workflow_run_status(
    ...
    async def process_self_update_step(
        self, staging_run: StagingRun, step: StagingRunStep, db: Session
    ):
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if result.get("update_status") == "updated":
                        commit_info = (
                            await self.self_update_manager.get_current_commit_info()
                        )
    ...
    async def process_kernel_tree_step(
        self, staging_run: StagingRun, step: StagingRunStep, db: Session
    ):
        ...
        if step.status == StagingStepStatus.PENDING:
            ...
            try:
                ...
                if staging_run.kernel_tree == "none":
                    ...
                    step.details = json.dumps(
                        {"skipped": True, "reason": "User selected 'None' option"}
                    )
                    print(
                        f"DEBUG: Skipping kernel tree update for staging run {staging_run.id}"
                    )
                    db.commit()
                    return
                    ...
                    staging_run.kernel_tree = (
                        kernel_tree  # Update to actual selected tree
                    )
                    ...
                    print(
                        f"DEBUG: Using specifically selected kernel tree: {kernel_tree}"
                    )
                else:
                    ...
                    step.end_time = datetime.utcnow()
                    step.error_message = (
                        f"Invalid kernel tree configuration: {staging_run.kernel_tree}"
                    )
                    print(
                        f"ERROR: Invalid kernel tree value in staging run {staging_run.id}: {staging_run.kernel_tree}"
                    )
                    db.commit()
                    return
    ...
    async def process_api_pipeline_step(
        self, staging_run: StagingRun, step: StagingRunStep, db: Session
    ):
    ...
    async def complete_staging_run(
        ...
        staging_run.status = (
            StagingRunStatus.COMPLETED if success else StagingRunStatus.FAILED
        )
        ...
        if not success:
            failed_step = next(
                (s for s in staging_run.steps if s.status == StagingStepStatus.FAILED),
                None,
            )
            if failed_step:
        ...
        if discord_webhook:
            try:
                ...
                if staging_run.start_time and staging_run.end_time:
                    duration_seconds = (
                        staging_run.end_time - staging_run.start_time
                    ).total_seconds()
            ...
            if step.status == StagingStepStatus.RUNNING:
                ...
                if step.start_time:
                    ...
                    if running_duration.total_seconds() > (timeout_minutes * 60):
                        print(
                            f"Detected stuck step {step.step_type.value} in run #{staging_run.id}"
                        )
                        print(
                            f"Step has been running for {running_duration.total_seconds()/60:.1f} minutes (timeout: {timeout_minutes}m)"
                        )

                        # For GitHub workflow steps, check if they're actually still running
                        if (
                            step.step_type == StagingStepType.GITHUB_WORKFLOW
                            and step.github_actions_id
                        ):
                            try:
                                if self.github_manager:
                                    ...
                                    if status["status"] == "completed":
                                        print(
                                            f"GitHub workflow {step.github_actions_id} completed but wasn't processed - recovering"
                                        )
                                        continue  # Let normal processing handle this
                                    elif status["status"] == "in_progress":
                                        print(
                                            f"GitHub workflow {step.github_actions_id} is still running - extending timeout"
                                        )
                        ...
                        print(
                            f"Marked step {step.step_type.value} as failed due to timeout"
                        )
    ...
    async def startup_recovery(self):
        ...
        try:
            ...
            for staging_run in running_stagings:
                print(
                    f"Checking staging run #{staging_run.id} for steps that need startup recovery"
                )
                recovered_any = False
                ...
                for step in staging_run.steps:
                    if step.status == StagingStepStatus.RUNNING:
                        ...
                        # GitHub workflows might still be running, so we'll let the normal recovery handle them
                            ...
                            print(
                                f"GitHub workflow step found - will be checked by normal recovery process"
                            )

                if recovered_any:
                    db.commit()
                    print(
                        f"Recovered {sum(1 for s in staging_run.steps if s.status == StagingStepStatus.FAILED and 'application restart' in (s.error_message or ''))} steps for staging run #{staging_run.id}"
                    )
    ...
    def _is_partial_success(
        self, status: Dict[str, Any], jobs_summary: Dict[str, Any]
    ) -> bool:
        """
