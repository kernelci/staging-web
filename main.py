...
async def staging_run_detail(
    ...
    if request.query_params.get("error") == "already_running":
        error_message = (
            "Cannot start a new staging run while this one is still running."
        )
...
async def users_page(
    ...
    return templates.TemplateResponse(
        "users.html", {"request": request, "user": current_user, "users": users}
    )
        ...
        if discord_webhook:
            try:
                await discord_webhook.send_staging_start(
                    current_user.username, staging_run.id
                )
            except Exception as e:
...
async def cancel_staging_run(
    ...
    if current_user.id != staging_run.user_id and current_user.role not in [
        UserRole.ADMIN,
        UserRole.MAINTAINER,
    ]:
        raise HTTPException(
    ...
    real_ip = get_real_ip(request)
    print(
        f"User '{current_user.username}' cancelling staging run #{run_id} from IP {real_ip}"
    )
    ...
    try:
        ...
        for step in staging_run.steps:
            if (
                ...
                and step.status == StagingStepStatus.RUNNING
                ...
                if github_token:
                    ...
                    cancelled = await github_manager.cancel_workflow_run(
                        step.github_actions_id
                    )
                    if cancelled:
                        print(
                            f"Cancelled GitHub workflow {step.github_actions_id} for step {step.id}"
                        )
                    else:
                        print(
                            f"Failed to cancel GitHub workflow {step.github_actions_id} for step {step.id}"
                        )
        ...
        if discord_webhook:
            try:
                ...
                await discord_webhook.send_staging_complete(
                    current_user.username, staging_run.id, "cancelled", duration
                )
            except Exception as e:
                print(f"Failed to send Discord notification: {e}")
