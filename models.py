...
class StagingRun(Base):
    ...
    status = Column(
        Enum(StagingRunStatus), nullable=False, default=StagingRunStatus.PENDING
    )
    initiated_via = Column(
        Enum(InitiatedVia), nullable=False, default=InitiatedVia.MANUAL
    )
    ...
    steps = relationship(
        "StagingRunStep", back_populates="staging_run", cascade="all, delete-orphan"
    )
...
class StagingRunStep(Base):
    ...
    status = Column(
        Enum(StagingStepStatus), nullable=False, default=StagingStepStatus.PENDING
    )
